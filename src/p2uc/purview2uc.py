      ...
      if(len(cfn_list) >0):
        for cfn in cfn_list:
          ...
          if(tech_classification == "MICROSOFT.POWERBI.ENDORSEMENT"):
            ...
            classification = self.sanitize_classification(classifcation_map[tech_classification])
            result = stmt + f"{column_name} SET TAGS ('classification_{classification}');"
          print(f"{result}")
    ...
    if (table['typeName'] == 'azure_synapse_dedicated_sql_table'):
      ...
      for cfn in cfns:
        if(cfn['typeName'] == "MICROSOFT.POWERBI.ENDORSEMENT"):
          ...
          readable_classification = self.sanitize_classification(classifcation_map[tech_classification])
